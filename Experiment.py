...
class Experiment():
    ...
    def _setRecordingVectors(self, secList):
        ...
        RecordingStruct = namedtuple("RecordingStruct", "ribV inhV gExc gInh gKv1_2 gHCN2 gCa") #create a datastructure to recording vectors
        ...
        for syn in self.model.excSyns.syn:
            rec.gExc.append( h.Vector().record(syn._ref_g) )
    ...
    def iClampSineWave(self, frequency = 1, baselineI = 0, amplitudeI = 0, start = 1000):
        sinDuration = self.tstop - start
        numSinPoints = round(sinDuration / self.model.h.dt)
        t = np.linspace(0, sinDuration/1000, numSinPoints) # time in seconds
        sin = np.sin(frequency * t * 2* np.pi) * amplitudeI + baselineI
        
        
        
        trace = np.zeros(round(self.tstop/self.model.h.dt))
        trace[len(trace)-len(sin) : len(trace)] = sin
        
        plt.plot(np.linspace(0,self.tstop,len(trace)), trace)
        plt.title("played current trace")
        plt.show()
        
        trace = self.model.h.Vector(trace)

        trace.play(self.iClamp._ref_amp, self.model.h.dt)       
